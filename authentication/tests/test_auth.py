...
class TestUserViewsetCreation:
    ...
    def test_admin_can_get_all_instructors(
        self, api_client, admin_user, instructor_fixture
    ):
        api_client.force_authenticate(user=admin_user)
    ...
    def test_staff_can_create_student_account(
        self, api_client, user_factory_fixture, program_factory_fixture
    ):
    ...
    def test_student_is_added_to_course_on_registration(
        self, api_client, admin_user, program_factory_fixture, course_factory_fixture
    ):
        program = program_factory_fixture(duration=3)
        course = course_factory_fixture(
            level=1, duration=1, name=f"{program.name}-basic", program=program
        )
        request_data = TestHelper().generate_test_data("student", program.id)
        api_client.force_authenticate(user=admin_user)
        unique = request_data["user"]["email"]
...
class TestAccounts:
    def test_forget_password_valid_email(self, api_client, user_factory_fixture):
        ...
        assert (
            response.data["message"] == "We have sent you a link to reset your password"
        )
    ...
    def test_change_password_password_validation_failure(
        self, api_client, generate_reset_token
    ):
        url = reverse("users-change-password")
        weak_password = "123"
        user, token = generate_reset_token
...
class TestStudent:
    def test_list_student_endpoint(
        self, api_client, admin_user, student_factory_fixture
    ):
...
class TestInstructor:
    def test_instructor_can_see_all_his_active_sessions(
        self, api_client, instructor_fixture, program_with_course_fixture
    ):
    ...
    def test_instructor_cannot_see_another_instructor_sessions(
        self, api_client, instructor_fixture, program_with_course_fixture
    ):
    ...
    def test_filter_instructors_by_skill(
        self,
        api_client,
        admin_user,
        instructor_fixture,
        skill_fixture,
        instructor_skills_fixture,
    ):
        from utils.util_functions import reverse_with_querystring
        # Create a set of shared skills
        python_skill = skill_fixture.create(name="Python")
        # Create 10 instructors
        py_instructors = instructor_fixture.create_batch(10)
        instructors_with_skills = instructor_skills_fixture.create_batch(20)

        # Assign skills to each instructor
        for instructor in py_instructors:
            instructor_skills_fixture(instructor_id=instructor, skill_id=python_skill)
        ...
        TestStrategyRunner.execute(
            ListStrategy(
                api_client,
                reverse_with_querystring("instructors-list", query_kwargs={"skill": "Python"}),
                models.Instructor,
                [
                    "id",
                    "fullname",
